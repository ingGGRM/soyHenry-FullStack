...
function apilarPantalones(array, talla) {
	// Tu código aquí:
	var restantes = new Stack();
	var encontrados = new Stack();

	array.forEach((item) => {
		if (item === talla) encontrados.push(item);
		else restantes.push(item);
	});

	return encontrados.size() === 0
		? false
		: {
				pantalonesEncontrados: encontrados,
				pantalonesRestantes: restantes,
		  };
}
